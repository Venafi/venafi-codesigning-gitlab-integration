...
class SigntoolSignCommand:
    ...
    def run(self):
        ...
        try:
            ...
            if self.config.sign_with == 'signtool':
                self._invoke_signtool()
            elif self.config.sign_with == 'powershell':
                self._invoke_powershell()
            else:
                f'Unknown signing tooll: {self.config.sign_with}'
        finally:
            self._logout_tpp()
    ...
    def _invoke_powershell(self):
        ...
        utils.invoke_command(
            self.logger,
            f'Signing with powershell: {self.config.input_path}',

...
class CosignVerifyCommand:
    def __init__(self, logger, config: CosignVerifyConfig):
    ...
    def _invoke_cosign(self):
        ...
        utils.invoke_command(
            self.logger,
            'Verifying with cosign: %s' % (self.config.image,),
            "Successfully verified '%s'." % (self.config.image,),
            "Error verifying '%s'" % (self.config.image,),
            'cosign',
            print_output_on_success=False,
            command=command,
            env=self.session_env
...
def main():
    try:
        ...
        config = CosignVerifyConfig.from_env()
        command = CosignVerifyCommand(logging.getLogger(), config)
